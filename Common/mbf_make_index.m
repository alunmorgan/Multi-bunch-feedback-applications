...
if isempty(wanted_datasets_type)
    ...
    ok = zeros(length(wanted_datasets_type), 1);
    file_time = cell(length(wanted_datasets_type), 1);
    file_name = cell(length(wanted_datasets_type), 1);
    for kse = 1:length(wanted_datasets_type) %parfor chokes the server
        try % handling corrupted input files
...
%             disp(fieldnames(temp))
    ...
    toc
    save(fullfile(root_string, [application_type, '_', index_name]), 'file_index')
end %if