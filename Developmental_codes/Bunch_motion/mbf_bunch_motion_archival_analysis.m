function [dataset, times, experimental_setup] = ...
    mbf_bunch_motion_archival_analysis(data_requested, varargin)
...
for nd = length(data_requested):-1:1
    ...
    data = mbf_bunch_motion_analysis(data_requested{nd});
    ...
    for st = 1:length(states)
        if nd == length(data_requested)
            temp_size = size(data.(states{st}));
            dataset.(states{st}) = NaN(nd, temp_size(1), temp_size(2));
        end %if
        dataset.(states{st})(nd,:,:) = data.(states{st});
    end %for
    fprintf('.')
end %for
fprintf('\n')
...
if strcmp(p.Results.analysis_type, 'parameter_sweep')
    ...
    [dataset, experimental_setup.param] = ...
        ...
        dataset, param, p.Results.parameter_step);

elseif strcmp(p.Results.analysis_type, 'average')
    warning('Archive:bunch_motion:setting','Ignoring the last two parameters as "average" is set')
