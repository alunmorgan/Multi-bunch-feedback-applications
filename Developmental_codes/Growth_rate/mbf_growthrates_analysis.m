function [poly_data, frequency_shifts] = mbf_growthrates_analysis(exp_data, varargin)
% takes the data from mbf growth rates capture and fits it with a series of
...
end_of_active = end_of_growth + exp_data.act_turns; %in samples
...
active_samples = (end_of_growth + 1):end_of_active;
active_turns = growth_turns(end) + (active_samples - growth_samples(end)) * act_dwell; %in turns
...
for nq = 1:n_modes 
    %% split up the data into growth and active damping.
    data_mode = data(nq,:);
    ...
    [s2, delta2, p2] = get_damping(active_turns, ad_data, active_override, length_averaging, adv_fitting,threshold_value);
...
% axis 1 is mode, axis 2 is expermental state (natural excitation, active damping). axis 3 is damping time, offset and fractional error.
