...
for gd = 1:size(PPRE_data.scan, 1)
    for hum = 1:size(PPRE_data.scan, 2)
        for snf = 1:size(PPRE_data.scan, 3)
            data_out.emittance_x(gd, hum, snf) = PPRE_data.scan{gd, hum, snf}.emittance{1}.hemit;
            data_out.emittance_y(gd, hum, snf) = PPRE_data.scan{gd, hum, snf}.emittance{1}.veimt;
            data_out.beam_sizes_P1(gd, hum, snf) = PPRE_data.scan{gd, hum, snf}.beam_sizes{1}.P1_sigy;
            data_out.beam_sizes_P1(gd, hum, snf) = PPRE_data.scan{gd, hum, snf}.beam_sizes{1}.P2_sigy;
