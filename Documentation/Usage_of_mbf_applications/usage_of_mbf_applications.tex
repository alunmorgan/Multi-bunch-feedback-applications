\documentclass{report}
\usepackage{graphicx} % Required for inserting images
%\usepackage{minted}
\usepackage[outputdir=../../]{minted} % for Overleaf compatibility
\usepackage{multirow}
\usepackage{a4wide}
\usepackage{fancyhdr}
\usepackage{amsfonts}
\usepackage{amsmath}
\usepackage{subcaption}
\usepackage{tcolorbox}
\usepackage{color}
\pagestyle{fancy}
\newcommand{\docTitle}{Multi bunch feedback application software}
\newcommand\abs[1]{\left|#1\right|}
\rhead{\docTitle}
\lhead{}
\usepackage{hyperref}
\hypersetup{
    colorlinks=true,
    linkcolor=blue,
    filecolor=magenta,      
    urlcolor=blue,
    pdftitle={Overleaf Example},
    pdfpagemode=FullScreen,
    }

\begin{document}
\title{\docTitle}
\author{Alun Morgan}
\date{June 2026}

\maketitle

\tableofcontents
\clearpage

\chapter{Overview}
\section{Conceptual descriptions}
The hardware for multi bunch feedback has many different possible configurations. Broadly speaking a feedback system has a frontend which takes in the signals from the pickups, and does the initial analogue signal conditioning. These signals are then fed into digital systems for each of the three planes (x,y,s). These digital systems calculate the feedback required and send signals to analogue drive electronics in order to excite the beam in the given plane. This is summarised in figure \ref{system_overview}

\begin{figure}[hbt]
\begin{center}
  \includegraphics[width=0.5\textwidth]{top_level_system_overview.png}\\
  \caption{System overview}\label{system_overview}
\end{center}
\end{figure}

In order to make such a system more easily usable for a wide range of system users, a suite of task specific applications has been developed. The aim of the suite is to abstract away the implementation detail of the hardware and thus allow the users to put their attention on type of measurement they want to do. This abstraction also aids in the development of bespoke measurements for specific investigations.

\section{Common application behaviour}
All these codes save the relevant data and graphs to a file structure based on the date, and given unique names based on the date and time the application was run. The root path for this structure is defined in \verb|mbf_system_config.m|, as is the harmonic number of the machine under study. In general all machine/deployment parameters are set in this file. The rest of the codebase it written to adapt to what is set here.

In addition to the system data from the multi bunch hardware, more general machine parameters are also captured to enable machine studies (e.g current dependence). This additional data is set in \verb|machine_environment.m|.

\section{Frontend Optimisation}
The frontend as an analogue RF system which allows control of two aspects. First, adjusting the gain of the signals so that we can use the appropriate amount of the ADC input range. Second, in order to work optimally the signals need to be matched in timing/phase with the beam phase (not the RF phase) and be tuned so that the sampling point of the ADC coincides with the maximum signal from each bunch.

The input level is decided by which operational condition you want to optimise for. Common options are injection, a uniform fill, or camshaft / hybrid mode where some buckets are filled with more charge. ADCs usually have a range over which they are linear and then a range beyond which where they have a non-linear response, eventually saturating. It may be that you want normal operation to be in the linear region but you can afford the injection to be in the non-linear region as long as it is below saturation. The general approach to setting the frontend gains is that you put the machine in the desired operational condition and adjust the frontend gains to bring the signal level into the correct region of the ADC range () more details on how to do this are in section  \ref{}.

The first timing requirement of aligning to beam phase is done by phase shifting the input signal. At its most basic this comprises of a single phase shifter for each channel which both allows each channel to be adjusted to follow changes in beam phase, and to allow the axis to be  tuned in order to account for differences in the phase delay of that channel.

The practical implementation will depend on the details of your frontend hardware.
However is often realised as a single LO phase shifter which is common to all axes and handles the beam phase adjustments. meanwhile each channel has an individual additional phase shifter to tune out the systemic differences in phase delay between the channels. Such a setup is shown in figure \ref{fig:phase_shifters}.
Another variation is replacing the LO phase shifter with an automatic beam phase following system such as a DORIS ((Delayed Orbit Reference Improvement Scheme) unit which locks the MBF to a fixed relationship to beam phase rather than the RF phase. 
DORIS will lock if there is more than 3~mA in the storage ring.

The second timing requirement is done by adjusting the sampling clock until a maximum signal is obtained with a minimum signal leakage to adjacent RF buckets. See figure \ref{fig:frontend_clock_phase_scan} for an example.
\begin{figure}[hbt]
    \centering
        \includegraphics[width=0.9\textwidth]{phase_shifters_overview.png}
        \caption{phase shifters}
        \label{fig:phase_shifters}
\end{figure}

\begin{tcolorbox}
\textbf{Note: }The Libera bunch by bunch frontend seems to get into problems when scanning the LO phase shifter. It is probably best not to scan the LO phase if you have that frontend.
\end{tcolorbox}
\clearpage
\subsection{Measurements}
\subsubsection{DORIS scans}
Scan the phase of the DORIS unit to identify an optimum target value for the feedback loop.

Basic usage:
\begin{minted}{matlab}
DORIS_target_phase_scan(<bunch>)
\end{minted}

\subsubsection{Frontend scans}
Scan the relevant phase shifter or clock delay while monitoring the measure signal amplitude. The resulting graphs will allow appropriate working points to be set.
\color{red}{ADD SOMETHING ABOUT AMPLITUDE SCANS}
\color{black}

Basic usage:
\begin{minted}{matlab}
BBBFE_full_frontend_checkout
\end{minted}

\subsubsection{Mode scans}
A mode scan is a sequential excitation of the harmonics of the tune frequency. This is mainly a system check in order to identify any performance limitations or degradation.

Basic usage:
\begin{minted}{matlab}
modescan_all('x')
\end{minted}

\subsubsection{Spectra}
This code captures the requested number of turns from the requested axis and returns the FFT of the centroid motion of each bunch. This is useful for identifying tunes, tune leakage from one axis to another, also for classifying instabilities.
\color{red}{EXTEND ANALYSIS TO EXTRACT TUNE LEAKAGE EXPLICITLY}
\color{black}

Basic usage:
\begin{minted}{matlab}
mbf_spectrum_all('x')
\end{minted}

\subsubsection{Growdamp}
The measurement comprises a series of measurements at set harmonics of the tune frequency capturing the evolution of bunch both with and without feedback active.
This can be with or without excitation. The former is the standard measurement of response to a given excitation. The latter is useful to find which modes the machine naturally excites.

The excitation is usually on the tune but can be set to follow a sideband or even a fixed point in tune space. Tune excitation is used to investigate the behaviour of the bunch centroid while sideband excitation is used to investigate then effect on the bunch size

Basic usage:
\begin{minted}{matlab}
growdamp_all('x')
\end{minted}

\subsubsection{Emittance control}
This uses one of the oscillators to excite the selected bunches at one of the sideband frequencies. This has the effect of blowing up the beam.
Basic usage:
\begin{minted}{matlab}

\end{minted}

\subsubsection{Bunch motion capture}
The three systems are setup to be ready to capture on the next external trigger. Once that trigger arrives, the three systems simultaneously capture bunch centroid motion. This is useful for capturing injection transients. 
Basic usage:
\begin{minted}{matlab}
\end{minted}

\chapter{System setup}
\section{Code location }
The code is released under an MIT licence. In order to use it please grab it from the \href{https://github.com/alunmorgan/Multi-bunch-feedback-applications}{code repository}
or use the following command on the system command line in order to copy it to your current location:
\begin{verbatim}
git clone https://github.com/alunmorgan/Multi-bunch-feedback-applications.git
\end{verbatim}
and add the resulting folder to your Matlab path. If you find odd behaviour check that you do not have any other code in the Matlab path as you could have name clashes.

If you have already cloned the repository navigate to the folder and run 
\begin{verbatim}
git pull
\end{verbatim}
This will ensure you have the most up to date version of the code.

This and related documentation can be found in the \href{https://github.com/alunmorgan/Multi-bunch-feedback-applications/Documentation}{documentation folder in the repository}

\section{MBF frontend open loop setup}
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=\textbf{Machine settings}]
  Insertion devices (IDs) need to be set to injection gaps.  
\end{tcolorbox}
 
\subsection{DORIS phase}
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=\textbf{Machine settings}]
Inject a full fill 10~mA.
\end{tcolorbox}
\subsubsection{DORIS phase scan}
Before running the code to scan the DORIS phase, select a single bunch to look at.
Make sure the single bunch location has charge in it.

The following code will scan the phase of the DORIS unit to identify an optimum value. It will select bucket 400 as the target single bunch location.
\begin{minted}{matlab}
    >> DORIS_target_phase_scan(400) 
\end{minted}
If you want to check that the settings are changing as expected then look at the screen in figure \ref{fig:DORIS_settings} (Diagnostics $\rightarrow$ SR $\rightarrow$ DORIS).
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{DORIS_settings.png}
    \caption{DORIS settings}
    \label{fig:DORIS_settings}
\end{figure}

Figure \ref{fig:DORIS_phase_scan} shows an example of the output.
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\linewidth]{DORIS_scan.png}
    \caption{In this case $75^\circ$ looks to be a reasonable working point.}
    \label{fig:DORIS_phase_scan}
\end{figure}

The optimum value is the phase where one gets the maximum signal of the excited bunch. Adjust the DORIS target phase to this phase value.
\clearpage
\subsection{Frontend Overview}
As both the amplification (gain) and timing (phase) adjustments can change the final input signal amplitude to the ADC it may be required to revisit one adjustment after a different one has been completed. For example you check and optimise the gains, then optimise the system phase and clock phase. This further increases the signal at the ADC and now required the gain to be reduced in order to stay in the linear range.
In the front end there are two phases (delays) we are interested in optimising. The system phase which determines how accurately our detectors are aligned with the target bunch, and the clock phase which determines how close to the peak of the signal we are sampling. 

Each axis in the frontend has individual gain control (fig \ref{fig:BBBFE_settings})
\begin{figure}[htb]
    \centering
    \includegraphics[width=0.6\linewidth]{BBBFE_settings.png}
    \caption{Frontend settings: Gains in input levels section. System phases in LO Phase shifts section  X=X, Y=Y S=IT. Clock phases in the MC Phase Shifts section X and Y=clk3, S=clk4}
    \label{fig:BBBFE_settings}
\end{figure}

The ADC graphs show fraction of full scale ($1 = 100\%$). See figure \ref{fig:example_adc_graphs}

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.75\linewidth]{Documentation//Usage_of_mbf_applications/adc_graphs_uniform_fill.png}
    \caption{Example ADC graphs. In the case of a 900 bunch (out of 936) fill. the ADC maximum is $\approx15\%$ for X and $\approx10\%$ for Y when no instability is present.}
    \label{fig:example_adc_graphs}
\end{figure}
\clearpage

\subsection{Frontend tuning}
Start with the frontend phases then move on to the frontend gains.
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=\textbf{Machine settings}]
\begin{itemize}
    \item {Inject a single bunch into a selected RF bucket (~0.2nC).}
     \item{if no sync press sync button as shown on figure \ref{fig:MBF_delays} }
     \begin{itemize}
         \item{(Diagnostics $\rightarrow$ SR $\rightarrow$ TMBF-X/TMBF-Y/LMBF $\rightarrow$ System $\rightarrow$ Delays)}
     \end{itemize}
       \end{itemize}
       \end{tcolorbox}

\subsubsection{Alignment to timing system}     
    \begin{itemize}
    \item{Compare the MBF ADC data and the fillpattern to the bunch number requested.}
    \begin{itemize}
        \item{Look at the Bunch motion standard deviation.}
        \begin{itemize}
        \item{Diagnostics $\rightarrow$ SR $\rightarrow$ TMBF-X/TMBF-Y/LMBF}
        \begin{figure}[htb]
            \centering
            \includegraphics[width=0.75\linewidth]{Documentation/Usage_of_mbf_applications/bunch_motion_standard_deviation.png}
            \caption{Bunch motion in normal operation \color{red}{NEED TO ADD SINGLE BUNCH EXAMPLE}}
            \label{fig:bunch_motion_standard_deviation_x}
        \end{figure}
        \end{itemize}
        \item{Look at fill pattern}
        \begin{itemize}
            \item{(Diagnostics $\rightarrow$ Photon counting system $\rightarrow$ fill) }
            \begin{figure}[htb]
                \centering
                \includegraphics[width=0.75\linewidth]{Documentation/Usage_of_mbf_applications/fill_pattern_900_bunch.png}
                \caption{fill pattern in normal operation \color{red}{NEED TO ADD SINGLE BUNCH EXAMPLE}}
                \label{fig:fill_pattern}
            \end{figure}
        \end{itemize}
    \end{itemize}
    \item{adjust MBF and fillpattern until they agree with the requested bunch number as given by the timing system.}
    \begin{itemize}
    \item{adjust MBF bunch value as shown on figure \ref{fig:MBF_delays}}
    \begin{figure}[htb]
        \centering
        \includegraphics[width=0.6\linewidth]{MBF_delays.png}
        \caption{MBF delay screen}
        \label{fig:MBF_delays}
    \end{figure}
       
    \end{itemize}
\end{itemize}

\clearpage


\subsubsection{System phase scan}
 
The following Matlab code will run a system phase scan on each axis sequentially. The graphs will be displayed, and the data is automatically stored. 

\begin{minted}{matlab}
    >> BBBFE_system_phase_scan('x', single_bunch_location)
    >> BBBFE_system_phase_scan('y', single_bunch_location)
    >> BBBFE_system_phase_scan('s', single_bunch_location)
\end{minted}

Figure \ref{fig:frontend_system_phase_scan} shows an example of the output. You are looking for the phase value with the largest gap between the excited bunch and each of the other bunches. But also fairly near the maximum excited bunch maximum amplitude.

\begin{figure}[hbt]
   \centering
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_system_phase_scan_x.png}
        \caption{Horizontal axis: $10^\circ$ looks to be a reasonable working point as you are trading of better selectivity for better signal to noise.}
        \label{fig:frontend_system_phase_scan_x}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_system_phase_scan_y.png}
        \caption{Vertical axis: $90^\circ$ looks to be a reasonable working point as you are near the maximum signal of the excited bunch and the selectivity for the preceding and following bunch's are also close to maximum. However you have lower ADC amplitude}
        \label{fig:frontend_system_phase_scan_y}
    \end{subfigure}
    
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_system_phase_scan_s.png}
        \caption{longitudinal axis: $20^\circ$ looks to be a reasonable working point for Q as it is near the zero crossing of the ADC phase. I then has to be $-70^\circ$ to keep the quadrature condition.}
        \label{fig:frontend_system_phase_scan_z}
    \end{subfigure}
    \caption{Example system phase results}
    \label{fig:frontend_system_phase_scan}
\end{figure}
\begin{tcolorbox}
\textbf{Note: }If the graphs are significantly asymmetric re-run the phase scan to check.

\textbf{Note:} If there are missing data points it can indicate that the DORIS system has unlocked. In this case you may need to inject a little more charge into the target bucket.
\end{tcolorbox}
\clearpage

\subsubsection{Clock phase scan} 
The following Matlab code will run a clock phase scan on each axis sequentially. The graphs will be displayed, and the data is automatically stored. 

\begin{minted}{matlab}
    >> BBBFE_clock_phase_scan('x', single_bunch_location)
    >> BBBFE_clock_phase_scan('y', single_bunch_location)
    >> BBBFE_clock_phase_scan('s', single_bunch_location)
\end{minted}
Figure \ref{fig:frontend_clock_phase_scan} shows an example of the output. You use the same assessment as for the system phase scans.
\begin{tcolorbox}
\textbf{Note:} X and Y share a clock so you have to optimise both axes together.
\end{tcolorbox}

\begin{figure}[hbt]
   \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{vlr_clock_phase_scan_x.png}
        \caption{Horizontal axis: $120^\circ$ looks to be a reasonable working point. lose to maximum signal and equalising the trade off between the preceding and following bunches.}
        \label{fig:frontend_clock_phase_scan_x}
    \end{subfigure}
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{vlr_clock_phase_scan_y.png}
        \caption{Vertical axis: $120^\circ$ looks to be a reasonable working point. As the clock for X and Y is common they need a solution which works for both. If they are significantly different you may have to go back and adjust the system phases.}
        \label{fig:frontend_clock_phase_scan_y}
    \end{subfigure}
    
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_clock_phase_scan_s.png}
        \caption{longitudinal axis: $160^\circ$ looks to be a reasonable working point.}
        \label{fig:frontend_clock_phase_scan_z}
    \end{subfigure}
    \caption{Example clock phase results}
    \label{fig:frontend_clock_phase_scan}
\end{figure}
\begin{tcolorbox}
\textbf{Note:} Check the bunch standard deviation graphs to check that the motion is still only in a single bucket. If it is split over two then you may have to choose an alternative working phase.
\end{tcolorbox}
\clearpage
\subsubsection{frontend gain scan}
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=\textbf{Machine settings}]
Put the machine in the desired condition (i.e. uniform fill)
\end{tcolorbox}
Adjust the gain on each channel so that the level on the ADC is $10 - 15\%$ with no instabilities present. 

\begin{tcolorbox}
Once the system phase, clock phase and frontend gain are optimised then rerun all measurements to make sure that later adjustments have not disturbed earlier ones. If all is good then the initial tuning of the front end is complete.
\end{tcolorbox}
The next step is to close the feedback loop.
\clearpage
\section{MBF closed loop setup}

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=\textbf{Machine settings}]
\begin{itemize}
    \item {From an empty machine Inject to ~50~mA into all bunches.}
    \begin{itemize}
    \item{this should give sufficient signal while staying below the instability threshold}
    \item{Check there is no activity on the standard deviation of motion graph (Figure \ref{fig:bunch_motion})}
    \begin{figure}[h]
        \centering
        \includegraphics[width=0.5\linewidth]{bunch_motion_standard_deviation.png}
        \caption{If you see distortion of the fill pattern or large amounts of jumping then you are probably in an unstable region. Drop the beam and refill to a lower current.}
        \label{fig:bunch_motion}
    \end{figure}
    \end{itemize}
    \item{Correct tune values to nominal.}
    \begin{itemize}
        \item{If you are struggling to get a reliable tune value try}
        \begin{itemize}
            \item{increasing the excitation (Figure \ref{fig:change_tune_excitation})}
              \begin{itemize}
            \item{(Diagnostics $\rightarrow$ SR $\rightarrow$ TMBF-X/TMBF-Y/LMBF  $\rightarrow$ System  $\rightarrow$ Control)}
             \end{itemize}
            \item{changing from fitted to maximum (Figure \ref{fig:tune_measurement_settings})}
              \begin{itemize}
            \item{(Diagnostics $\rightarrow$ Summary $\rightarrow$ Click tune value )}
             \end{itemize}
        \end{itemize}
    \end{itemize}
    \item{Turn tune feedback off once tunes are nominal}
    \item{Correct orbit using fast orbit feedback (FOFB). This can be left on}
\end{itemize}
\end{tcolorbox}

\begin{figure}[ht]
    \centering
    \begin{subfigure}[b]{0.45\textwidth}
    \includegraphics[width=1\linewidth]{increasing_tune_excitation.png}
    \caption{Tune setup}
    \label{fig:change_tune_excitation}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=1\linewidth]{Documentation/Usage_of_mbf_applications/tune_measurement_screens.png}
    \caption{Tune measurement settings}
    \label{fig:tune_measurement_settings}
    \end{subfigure}
\end{figure}

\subsection{Initial measurement}
The following Matlab code will run a scan across all the harmonics of the tune on each axis sequentially. The graphs will be displayed, and the data is automatically stored.
\begin{minted}{matlab}
    >> modscan_all('x') 
    >> modscan_all('y') 
    >> modscan_all('s') 
\end{minted}

\subsection{Assessment} 

Use the modescan results to assess whether the closed loop delay is good enough.
 \begin{figure}[hbt]
   \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{Documentation/Usage_of_mbf_applications/vlr_modescan_x.png}
        \caption{In this case the phase is ranging from $-70^\circ$ to $50^\circ$ and there is almost no residual gradient along the modes. Although any further increase in this range would be concerning and would warrant further investigation.}
        \label{fig:example_modescan_x}
    \end{subfigure}
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{Documentation/Usage_of_mbf_applications/vlr_modescan_y.png}
        \caption{The phase is ranging from $-50^\circ$ to $50^\circ$ and there is almost no residual gradient along the modes indicating that the loop delay is close to the required value.}
        \label{fig:example_modescan_y}
    \end{subfigure}
\end{figure}

The amplitude should be fairly symmetric in the modes axis. If there is a significant change in the lowest modes only then check you have filled all bunches. The fewer bunches you fill the larger the distortion of the lower mode response becomes. 

The phase variation should be much less than $\pm90^\circ$ and the extents should be balanced between going above and below zero.

By taking a linear fit along the modes one can estimate the required electrical delay needing to be added or removed from the loop. Positive gradient means length must be added while negative gradient means length must be removed.

This error in the electrical length / delay / phase can be fixed in the following ways. 
\begin{itemize}
    \item {First see if a DAC delay can fix it (using the control panel shown in figure \ref{fig:DAC_delay_screen}). This is only available in 2ns steps.}
    \item {Otherwise, we must change cable lengths. The easiest place to do this is at the output port of the output amplifier. As shown in figure \ref{fig:output_amplifiers}}
\end{itemize}
Once the loop is closed for all modes the FIR phase can be adjusted to maximise the feedback efficacy

\subsection{How to tune the loop delay} 

The aim of this exercise is to get the extents of the phase trace comfortably fit within the $\pm90^\circ$ limits (similar to the result show in figure \ref{fig:example_modescan_y}). 

Rerun the modescan code after each change.
\begin{tcolorbox}
\textbf{Note:} The spec for the phase variation of the output amplifier alone is $\pm~20$.
\end{tcolorbox}

\subsubsection{Tuning the DAC timings}
If you see a large ($>2ns$) change over all the modes then the first thing to look at is the DAC timings. These are independent for each axis.

\subsubsection{Tuning the DAC delay} 
This allows additional phase (delay) to be added to the closed loop.
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.5\linewidth]{DAC_delay.png}
    \caption{DAC delay control screen}
    \label{fig:DAC_delay_screen}
\end{figure}
\begin{tcolorbox}
\textbf{Note:} The DAC delay is shared between X and Y axes so it may be required to retune both.
\end{tcolorbox}

If delay needs to be reduced then the bunch setting can be reduced by one to reduce the overall delay by 2ns. Additional delay can be then added to reach the target delay.
Use DAC delay first to get as close as possible to the target performance. Any remaining adjustment will have to be done with physical cable length changes. 
However one should check the bunch position with respect to the timing system once the loop has been successfully closed for all modes.

\subsubsection{Tuning the cable length} 
Modify the cable length until the extents of the phase trace comfortably fit within the limits.  
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.6\linewidth]{amp_front.png}
    \caption{Output amplifiers showing the RF cables to adjust}
    \label{fig:output_amplifiers}
\end{figure}
A large phase variation even when the overall gradient is close to zero indicates that a component is failing – probably the output amplifier. So, a hardware change and a retuning of the frontend is required. 
\subsubsection{Tuning the FIR phase}
The overall phase of the feedback loop should be 180 degrees. So the last step is to change the FIR phase setting so the readback value is close to ($\pm~20$) 180.

At this point the full loop delay should be optimised
\clearpage

\section{MBF measurements}
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=\textbf{Machine settings}]
\begin{itemize}
    \item {Inject to 30 - 60mA $\geq900$ bunches (Ideally it should be 936 bunches and below the instability threshold)}
    \item{Set FIR gains on the MBF feedback to nominal values}
\end{itemize}
\end{tcolorbox}
\subsection{Code to run}

The following Matlab code will run modescan, growdamp and spectra measurements for each axis sequentially. The data is stored automatically 
\begin{verbatim}
    >> mbf_startup_tests 
\end{verbatim}
\vspace{2mm}
The following explains how to run each separate standard measurement. However there are some common inputs.
\begin{itemize}
    \item Required
    \begin{itemize}
        \item{\begin{minted}{matlab}
'mbf_axis', 'x' or 'x' or 's'. 
\end{minted}
The axis being measured.}
        \end{itemize}
    \item Optional
    \begin{itemize}
        \item{\begin{minted}{matlab}
'plotting', 'yes' or 'no'. Default is yes
\end{minted}
Determines if the plotting is displayed. The raw data is saved in either case.}
        \item{\begin{minted}{matlab}
'additional_save_location', string representing a filepath. Default is NaN
\end{minted}
Saves the data in the requested location in addition to the standard location.}
        \item{\begin{minted}{matlab}
'auto_setup', 'yes' or 'no'. Default is yes
\end{minted}
This determines whether to use the main scripts to reset the system to a known state. However this is dangerous if the machine is not in an intrinsically stable condition.}
    \end{itemize}
\end{itemize}

\subsubsection{Growdamp}
The top level code to run a growdamp measurement for each axis is; 
\begin{minted}{matlab}
    >> growdamp_all('x') 
    >> growdamp_all('y') 
    >> growdamp_all('s') 
\end{minted}

The parameters for the length of excitation and  capture are set in \verb|mbf_growdamp_config.m|.
 The \verb|mbf_growdamp_setup| code sets up the hardware for the correct type of measurement. 
 
 \verb|mbf_growdamp_capture| triggers the measurement, captures the data and stores it in the file structure defined in \verb|mbf_system_config|. If plotting is selected the code shows the last saved dataset.
 
Additional optional settings
\begin{itemize}
\item \verb|excitation(str)|: sets whether the measurement uses an excitation to drive a resonance or if the measurement just uses the natural beam coupling. Defaults to 'yes'
\item \verb|excitation_location(str)|: The location of the excitation in tune space. The default value is 'tune'.
\item \verb|excitation_manual(float)|: if \verb|excitation_location| is set to 'manual' then this defines the location of the excitation in tune space.
\item \verb|pll_tracking (str)|: Sets if the excitation frequency follows the tune jitter. Default is no
\item \verb|pll_bunches (float)|: bunches the pll is active on.
\item \verb|pll_guard_bunches (float)|: the number of bunches surrounding the pll bunches for which feedback is switched off.
\item \verb|bunch_monitor(list(int)|): This determines which bunches are monitored during the measurement. Defaults to and array of ones which means all bunches.
\item \verb|capture_full_bunch_motion(str)|: Captures the centroid motion for the duration of the measurement. Defaults to no as this is a lot of data. 
\end{itemize}
\subsubsection{Modescan}
The top level code to run a modescan measurement for each axis is; 
\begin{minted}{matlab}
    >> modescan_all('x') 
    >> modescan_all('y') 
    >> modescan_all('s') 
\end{minted}

The \verb|mbf_modescan_setup| code sets up the hardware for the correct type of measurement. \verb|mbf_modescan_capture| triggers the measurement, captures the data and stores it in the file structure defined in \verb|mbf_system_config|. 

Additional optional settings
\begin{itemize}
    \item \verb|n_repeats(int)|: How many times to repeat each measurement point.
    \item \verb|dwell (int)|: How many turns to stay at each measurement point.
    \item \verb|excitation_gain(float)|: level of the excitation.
\end{itemize}

\subsubsection{Spectra}
The top level code to run a modescan measurement for each axis is; 
\begin{minted}{matlab}
    >> mbf_spectrum_all('x') 
    >> mbf_spectrum_all('y') 
    >> mbf_spectrum_all('s') 
\end{minted}

The \verb|mbf_spectrum_setup| code sets up the hardware for the correct type of measurement. \verb|mbf_spectrum_capture| triggers the measurement, captures the data and stores it in the file structure defined in \verb|mbf_system_config|. 

Additional optional settings
\begin{itemize}
    \item \verb|n_turns (int)|: number of turns to capture. Default is 1000.
    \item \verb|repeat (int)|: Number of repeat points (improves the power resolution). Default is 1.
\end{itemize}

The MBF system is also used as an instrument for investigating the accelerator behaviour and the behaviour of the beam under different conditions. It can also be used as a driver for certain beam behaviours. These codes tend to be less well tested as they are often only used for particular investigations.
\subsection{Emittance control}
\subsection{Bunch motion}

 \verb|mbf_bunch_motion_setup| sets up all three systems to use the appropriate triggers. 
 
 \verb|mbf_bunch_motion_capture| arms the systems, and once triggered by an external trigger, will capture and store the data.
\subsection{Bunch cleaning}
\subsection{Chromaticity}
Uses the scan of tune and sidebands to find the chromaticity of the machine.
\subsection{Tune scan over modes}
Scan to allow optimisation of the tune measurement.

\chapter{MBF Archival retrieval}

\section{Introduction}

All the MBF codes store their data in a common location with programmatically constructed names. Therefore, it is possible to extract and compare this data. 

\section{Getting the latest data} 

The simplest thing is to look at the last results taken. To do this simply run the command below on the Matlab command line. 
\begin{minted}{matlab}
   >> visualise_latest_mbf_results 
\end{minted}
 This will return analysed results for all the standard measurements.
 
 There are optional flags to modify the standard behaviour.
 \begin{itemize}
     \item {\begin{minted}{matlab}
'save_graphs', 'yes', 'out_path', [your desired location])
\end{minted}
Save PNGs of the graphs at a location of your choosing.}
   \item {\begin{minted}{matlab}
'measurements', {'Growdamp', 'Modescan', 'Spectrum', 'system_phase_scan',...
'clock_phase_scan', 'DORIS_phase_scan'}
\end{minted}
Select only the measurements you are interested in.}
   \item {\begin{minted}{matlab}
'axes', {'x', 'y', 's'}
\end{minted}
Select only the axes you are interested in.}
 \end{itemize}
 
Figure \ref{fig:growdamp_example} is an example of the output for Growdamp, figure \ref{fig:modescan_example} of Modescan measurements and figure \ref{fig:spectrum_example} of spectral measurements.

 \begin{figure}[hbt]
   \centering
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_growdamp_x.png}
        \caption{Horizontal axis}
        \label{fig:growdamp_example_x}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_growdamp_y.png}
        \caption{Vertical axis}
        \label{fig:growdamp_example_y}
    \end{subfigure}
    
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_growdamp_s.png}
        \caption{longitudinal axis}
        \label{fig:growdamp_example_z}
    \end{subfigure}
    \caption{Example Growdamp results}
    \label{fig:growdamp_example}
\end{figure}

 \begin{figure}[hbt]
   \centering
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_modescan_x.png}
        \caption{Horizontal axis}
        \label{fig:modescan_example_x}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_modescan_y.png}
        \caption{Vertical axis}
        \label{fig:modescan_example_y}
    \end{subfigure}
    
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_modescan_s.png}
        \caption{longitudinal axis}
        \label{fig:modescan_example_z}
    \end{subfigure}
    \caption{Example modescan results}
    \label{fig:modescan_example}
\end{figure}

 The spectra graphs show the residual motion after all feedbacks have been applied.

 \begin{figure}[hbt]
   \centering
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_spectrum_x.png}
        \caption{Horizontal axis}
        \label{fig:spectrum_example_x}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_spectrum_y.png}
        \caption{Vertical axis}
        \label{fig:spectrum_example_y}
    \end{subfigure}
    
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_spectrum_s.png}
        \caption{longitudinal axis}
        \label{fig:spectrum_example_z}
    \end{subfigure}
    \caption{Example spectrum results- PLACEHOLDER}
    \label{fig:spectrum_example}
\end{figure}
\clearpage

\begin{figure}[hbt]
   \centering
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_system_phase_scan_x.png}
        \caption{Horizontal axis}
        \label{fig:system_phase_example_x}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_system_phase_scan_y.png}
        \caption{Vertical axis}
        \label{fig:system_phase_example_y}
    \end{subfigure}
    
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_system_phase_scan_s.png}
        \caption{longitudinal axis}
        \label{fig:system_phase_example_z}
    \end{subfigure}
    \caption{Example system phase results}
    \label{fig:system_phase_example}
\end{figure}


\begin{figure}[hbt]
   \centering
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_clock_phase_scan_x.png}
        \caption{Horizontal axis}
        \label{fig:clock_phase_example_x}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_clock_phase_scan_y.png}
        \caption{Vertical axis}
        \label{fig:clock_phase_example_y}
    \end{subfigure}
    
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{vlr_clock_phase_scan_s.png}
        \caption{longitudinal axis}
        \label{fig:clock_phase_example_z}
    \end{subfigure}
    \caption{Example clock phase results}
    \label{fig:clock_phase_example}
\end{figure}

\begin{figure}[hbt]
    \centering
    \includegraphics[width=0.5\linewidth]{Documentation//Usage_of_mbf_applications/DORIS_scan.png}
    \caption{Example DORIS phase scan}
    \label{fig:doris_phase_scan}
\end{figure}
\clearpage

\section{Getting any historic data} 

A more flexible way is to set up the time period you are interested in looking at and use the measurement specific retrieval tool. If there is only one dataset in the chosen date range then the graphs in the previous section are created. Otherwise alternative graphs are plotted which better allow the investigation of variations over time or variation with current for example.

Again there are common settings
\begin{itemize}
    \item Required
    \begin{itemize}
        \item{\begin{minted}{matlab}
ax, {'x', 'y', 's'}
\end{minted}
Select the axis of interest.}
    \end{itemize}
    \begin{itemize}
        \item{You can set up the period of interest by using a modified version the following example code. 
\begin{minted}{matlab}
>> start_datenum = datetime(2026, 04, 21, 15, 15, 00); 
>> end_datenum = datetime(2026, 04, 21, 18, 00, 00); 
>> date_range = [start_datenum, end_datenum]; 
\end{minted}
}
    \end{itemize}
    \begin{itemize}
        \item{filter conditions. As machine conditions can be different it is possible to filter on metadata values. Usually current range and RF frequency range. This is often useful to ensure that any measurements used are below instability thresholds. 
 }
    \end{itemize}
    \item Optional
     \begin{itemize}
        \item{\begin{minted}{matlab}
'bypass_index', 'yes' or 'no' Default is no
\end{minted}
The codes write data to index files for faster retrieval. If the file is corrupted this option allows the retrieval code to use the underlying data.}
    \end{itemize}
     \begin{itemize}
        \item {\begin{minted}{matlab}
'metadata_only', 'yes' or 'no' Default is no
\end{minted}
Useful to quickly find what measurement have been done.}
    \end{itemize}
     \begin{itemize}
        \item {\begin{minted}{matlab}
'analysis_type', 'collate' or 'sweep' Default is 'collate'
\end{minted}
Either just plot each dataset on top of one another, or sort them against a variable in the dataset.}
    \end{itemize}
     \begin{itemize}
        \item {\begin{minted}{matlab}
'sweep_parameter', any variable in the dataset Default is 'current'
\end{minted}
If analysis type is set to sweep this determines the variable to sweep against.}
    \end{itemize}
         \begin{itemize}
        \item {\begin{minted}{matlab}
'parameter_step', Default is 0.1
\end{minted}
In the sweep datasets with a sweep value closer than this are averaged.}
    \end{itemize}
\end{itemize}

An example of putting this together
\begin{minted}{matlab}
mbf_growdamp_archival_retrieval('y', 
[datetime(2026, 04, 21, 15, 15 ,00), datetime(2026, 04, 21, 18, 00, 00)],
{'current', [30 300]},
'analysis_type', 'sweep',
'sweep_parameter', 'current',
'parameter_step', 5)
\end{minted}
In this case we are asking for all data between 3pm and 6pm on 21st April 2026, but only if the current is between 30mA and 300mA. We are asking the output to be plotted as a current sweep with a minimum step of 5mA.

\subsection{Growdamp}
The following command will extract any measurements taken within the date range for the y axis and then plot them on top of one another. An example of this is shown in figure \ref{fig:growdamp_collate}.
\begin{minted}{matlab}
    >> mbf_growdamp_archival_retrieval('y',  date_range, {}); 
\end{minted}
\begin{figure}[ht]
    \centering
    \includegraphics[width=1\linewidth]{growdamp_collate.png}
    \caption{Collated growdamp results}
    \label{fig:growdamp_collate}
\end{figure}

This allows easy identification of changes in behaviour 

\subsubsection{Additional optional settings}
\begin{itemize}
    \item{\begin{minted}{matlab}
'active_override', number of turns to analyse, Default is NaN for all
\end{minted}
}
    \item{\begin{minted}{matlab}
'passive_override', number of turns to analyse, Default is NaN for all
\end{minted}
}
    \item{\begin{minted}{matlab}
'advanced_fitting', allows alternative fitting algorithm to be used, 
Default is 0 for basic fitting 
\end{minted}
}
    \item{\begin{minted}{matlab}
'debug', plots extra graphs to help identify the cause of odd behaviour, 
Default is 0 for off
\end{minted}
}
\end{itemize}

\subsubsection{Common filtering options}
\begin{itemize}
    \item{\begin{minted}{matlab}
'current', [30 300] The current range to include
\end{minted}
}
    \item{\begin{minted}{matlab}
 'excitation_setting', 'yes'or 'no' 
 This allows one to distinguish between experimanets with driven growth
 and the machines natural growth.
\end{minted}
}
    \item{\begin{minted}{matlab}
 'excitation_location', 'tune' or 'sideband' or 'manual' 
 This separates out different types of excitation. 
\end{minted}
}
\end{itemize}
\subsubsection{Examples}
One common set of measurements beyond simple comparisons is plotting the measurements against a changing machine parameter (usually current) 

Although it is possible to extract such relationships from the data just using the variation over time, it is more usual to have a dedicated measurement set for the purpose. In this case you would set the period to only contain data from that experiment. Then you would run the retrieval with the analysis type set to sweep and then set the appropriate parameter to sweep over (again it is usually current but any variable in the datasets can be used). Figure \ref{fig:growdamp_parameter_sweep} shows an example of this.
\begin{minted}{matlab}
    >> mbf_growdamp_archival_retrieval('y',...
    [datetime(2026, 04, 21, 15, 15 ,00), datetime(2026, 04, 21, 18, 00, 00)],...
    {'current', [30 300], 'excitation_setting', 'yes',...
    'excitation_location', 'tune'}, ...
    'analysis_type', 'sweep', 'parameter_step', 5)
\end{minted}
\begin{figure}[ht]
    \centering
    \includegraphics[width=1\linewidth]{growdamp_parameter_sweep.png}
    \caption{Growdamp results against a swept parameter. In this case the beam current. Note: These are placeholder graphs.}
    \label{fig:growdamp_parameter_sweep}
\end{figure}
At this point further analysis is possible. In the case of a current sweep, it is possible to predict the current which individual modes will go unstable. (figures \ref{fig:growdamp_damping_rates} and \ref{fig:growdamp_extrapolated_damping_rates}). 
\begin{figure}[ht]
    \centering
    \includegraphics[width=1\linewidth]{growdamp_damping_rates.png}
    \caption{Results from further analysis of the current dependent growdamp data.}
    \label{fig:growdamp_damping_rates}
\end{figure}
\begin{figure}[ht]
    \centering
    \includegraphics[width=1\linewidth]{growdamp_extrapolated_damping_rates.png}
    \caption{Results from further analysis of the current dependent growdamp data. Extrapolating to full operational current.}
    \label{fig:growdamp_extrapolated_damping_rates}
\end{figure}
\clearpage

\subsection{Modescan}
\subsubsection{Additional optional settings}
\begin{itemize}
    \item{None}
\end{itemize}
\subsubsection{Common filtering options}
\begin{itemize}
    \item{\begin{minted}{matlab}
'current', [30 300] The current range to include
\end{minted}
}
\end{itemize}
\subsubsection{Examples}
Modescan data retrieval is much the same as the growdamp retrieval. Once the date range is set up the following code will extract and display all results from the y axis (figure \ref{fig:modescan_collate}).  
\begin{minted}{matlab}
    >> mbf_modescan_archival_retrieval('y', ...
    [datetime(2026, 04, 21, 15, 15 ,00), datetime(2026, 04, 21, 18, 00, 00)],...
    {}, 'analysis_type', 'sweep', 'parameter_step', 5)
\end{minted}
 \begin{figure}[ht]
     \centering
     \includegraphics[width=1\linewidth]{modescan_collate.png}
     \caption{Collated modescan results}
     \label{fig:modescan_collate}
 \end{figure}



\subsection{Spectra}
\subsubsection{Additional optional settings}
\subsubsection{Common filtering options}
\begin{itemize}
    \item{\begin{minted}{matlab}
'current', [30 300] The current range to include
\end{minted}
}
\end{itemize}
\subsubsection{Examples}
Tune sweep over modes 
\clearpage

\end{document}




