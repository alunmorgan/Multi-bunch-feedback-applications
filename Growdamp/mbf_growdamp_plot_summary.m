function mbf_growdamp_plot_summary(poly_data, frequency_shifts, outputs)
...
%       outputs (str): 'passive' or 'both' Determines if teh active traces are
...
ax2 = subplot(3,1,3);
plot(x_plt_axis, circshift(frequency_shifts(:,1), -harmonic_number/2, 1), 'b')
if strcmp(outputs, 'both')
    hold on
    plot(x_plt_axis, circshift(frequency_shifts(:,2), -harmonic_number/2, 1), 'g')
    hold off
end %if
xlim([x_plt_axis(1) x_plt_axis(end)])
xlabel('Mode')
