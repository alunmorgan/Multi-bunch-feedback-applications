function [adc_mean, adc_max, adc_min, times, experimental_setup] = ...
    mbf_frontend_gain_scan_archival_analysis(data_requested, varargin)
...
for nd = length(data_requested):-1:1
    ...
    adc_mean(:,nd) = data_requested{nd}.adc_mean(:,1);
    adc_max(:,nd) = data_requested{nd}.adc_max(:,1);
    adc_min(:,nd) = data_requested{nd}.adc_min(:,1);
...
if strcmp(p.Results.analysis_type, 'parameter_sweep')
    ...
    [adc_mean, experimental_setup.param] = ...
        mbf_analysis_reorganise_for_parameter_sweep(...
        adc_mean', param, p.Results.parameter_step);
    [adc_max, experimental_setup.param] = ...
        mbf_analysis_reorganise_for_parameter_sweep(...
        adc_max', param, p.Results.parameter_step);
    [adc_min, experimental_setup.param] = ...
        mbf_analysis_reorganise_for_parameter_sweep(...
        adc_min', param, p.Results.parameter_step);

elseif strcmp(p.Results.analysis_type, 'average')
    warning('Archive:Frontend:Gain:setting','Ignoring the last two parameters as "average" is set')
    adc_mean = mean(adc_mean, 1, 'omitnan');
    adc_max = mean(adc_max, 1, 'omitnan');
    adc_min = mean(adc_min, 1, 'omitnan');
end %if
