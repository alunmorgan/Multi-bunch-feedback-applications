function conditioned_data = mbf_frontend_gain_scan_archival_retrieval(ax,...
...
%                                            structures.
%
% Example: mbf_frontend_gain_scan_archival_retrieval('x', [datetime(2023, 1, 1), datetime("now")])
...
selection_name = ['frontend_gain_scan_', ax, '_axis'];
...
if length(requested_data) == 1
   BBBFE_gain_scan_plotting(requested_data{1} )
else
    ...
    if isempty(conditioned_data)
        ...
        if strcmp(p.Results.analysis_type, 'collate')
            [adc_mean, adc_max, adc_min, times, setup] = ...
                mbf_frontend_gain_scan_archival_analysis(conditioned_data, 'analysis_type','collate');
        elseif strcmp(p.Results.analysis_type, 'sweep')
            [adc_mean, adc_max, adc_min, times, setup] = ...
                mbf_frontend_gain_scan_archival_analysis(conditioned_data, 'analysis_type','parameter_sweep', ...
                'sweep_parameter',p.Results.sweep_parameter,...
                'parameter_step', p.Results.parameter_step);
        end %if
        setup.axis = ax;
        mbf_frontend_gain_scan_archival_plotting(conditioned_data, adc_mean, adc_max, adc_min, times, setup);
    end %if
end %if