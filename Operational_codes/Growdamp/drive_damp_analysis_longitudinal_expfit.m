...
f=@(p,time) (p(1).*exp(-p(2).*time)+p(3)); % fit parameters are amplitude - p(1), damping - p(2), and "zero"-point offset - p(3)
opts = optimset('MaxFunEvals',50000, 'MaxIter',10000); % further "optimisation" of opts can make the fit faster...
for m=1:400
