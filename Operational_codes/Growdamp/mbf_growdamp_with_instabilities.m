...
fir_gain_x = get_variable([mbf_names.x, mbf_vars.FIR.gain]);
fir_gain_y = get_variable([mbf_names.y, mbf_vars.FIR.gain]);

% Get the tunes
tunes = get_all_tunes;

try
    set_variable([mbf_names.x, mbf_vars.FIR.gain], '0dB')
    [~] = growdamp_all('x',  'plotting', 'no', 'auto_setup', 'no', 'tunes', tunes);
    set_variable([mbf_names.x, mbf_vars.FIR.gain], fir_gain_x)
catch
    disp('Problem with Growdamp in X axis')
end %try
try
    set_variable([mbf_names.y, mbf_vars.FIR.gain], '0dB')
    [~] = growdamp_all('y',  'plotting', 'no', 'auto_setup', 'no', 'tunes', tunes);
    set_variable([mbf_names.y, mbf_vars.FIR.gain], fir_gain_y)
catch
