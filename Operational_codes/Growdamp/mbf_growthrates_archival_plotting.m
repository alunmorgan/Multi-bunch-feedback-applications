function mbf_growthrates_archival_plotting(requested_data, dr_growth, dr_active, error_growth, error_active, times, experimental_setup)
% Plots the data processed by mbf_growthrates_archival_analysis.
% Args:
%      dr_growth (numeric matrix): Growth rate.
%                                   (bunches vs datasets)
%      dr_active (numeric matrix): Passive damping rate.
%                                   (bunches vs datasets)
%      error_passive (numeric matrix): Error of the fit for the growth rate.
...
%
% Example: mbf_growdamp_archival_plotting(dr_growth, dr_active, error_growth, error_active, times, setup, selections, extents)
...
hold on
if strcmp(experimental_setup.anal_type, 'parameter_sweep')
    dr_growth = squeeze(dr_growth);
    dr_active = squeeze(dr_active);
    plot(x_plt_axis, dr_growth, 'LineWidth', 2)
    legend(graph_labels)
else
    populate_graph(dr_growth, years_input, times, x_plt_axis)
end %if
...
legend show
ymin = min(min(dr_growth,[],2));
if ymin > 0
    ymin = 0;
end %if
ymax = max(max(dr_growth,[],2));
ymax = ymax + ymax /10;
ylim([ymin ymax]);
grid on
hold off
...
if strcmp(experimental_setup.anal_type, 'parameter_sweep')
    ...
    hold on
    plot(experimental_setup.param, dr_growth')
    ...
    if strcmp(experimental_setup.sweep_parameter, 'current')
        ...
        hold on;
        for ks = 1:length(dr_growth)
            P = polyfit(experimental_setup.param',dr_growth(:,ks),1);
            ...
            plot(experimental_setup.param', dr_growth(:,ks),'ko', 'HandleVisibility', 'off');
            end %if
        end %for
        xlabel(experimental_setup.sweep_parameter)
        ylabel('Growth rates (1/turns)')
...
if nargin == 9
    figure
    ax3 = subplot(2,1,1);
    populate_graph(error_growth, years_input, times, x_plt_axis)
    title('Growth rate errors')
    xlabel('Mode')
    ylabel('Growth rate errors')
    ax4 = subplot(2,1,2);
