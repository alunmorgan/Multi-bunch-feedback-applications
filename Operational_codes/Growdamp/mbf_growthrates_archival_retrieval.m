function conditioned_data = mbf_growthrates_archival_retrieval(ax, date_range, varargin)
...
if strcmpi(ax, 'x')
    filter_name = 'Growthrates_x_axis';
elseif  strcmpi(ax, 'y')
    filter_name = 'Growthrates_y_axis';
elseif strcmpi(ax, 's')
    filter_name = 'Growthrates_s_axis';
else
...
if isempty(conditioned_data)
    ...
    if strcmp(p.Results.analysis_type, 'collate')
        [dr_growth, dr_active, error_growth, error_active, times, setup] = ...
            mbf_growthrates_archival_analysis(conditioned_data, 'analysis_type','collate',...
            'overrides', p.Results.overrides, ...
            'advanced_fitting', p.Results.advanced_fitting,...
            'debug', p.Results.debug);
    elseif strcmp(p.Results.analysis_type, 'sweep')
        [dr_growth, dr_active, error_growth, error_active, times, setup] = ...
            mbf_growthrates_archival_analysis(conditioned_data, 'analysis_type','parameter_sweep', ...
    ...
    setup.axis = ax;
    mbf_growthrates_archival_plotting(conditioned_data, dr_growth, dr_active, error_growth, error_active, times, setup);
end %if
