...
% bunch gains
% mbf_get_then_put([pv_head, BB.Gains], ones(1,harmonic_number));
...
elseif out_type == 1
    % FIR only
    mbf_get_then_put([pv_head, BB.FIR_gains], ones(1,harmonic_number) .* 1);
...
elseif out_type == 2
    % NCO only
    set_variable([pv_head, BB.FIR_set_disable],1)
    mbf_get_then_put([pv_head, BB.NCO1_gains], ones(1,harmonic_number) .* 1);
...
% SR23C-DI-TMBF-01:X:BUN1:FIR:SET_DISABLE_S
    set_variable([pv_head, BB.NCO1_disable],1)
    set_variable([pv_head, BB.NCO2_disable],1)
    mbf_get_then_put([pv_head, BB.SEQ_gains], ones(1,harmonic_number) .* 1);
...
% mbf_get_then_put([pv_head, BB.Output_types], ones(1,harmonic_number) .* out_type);
% select which FIR filter to use
mbf_get_then_put([pv_head, BB.FIR_select], ones(1,harmonic_number) .* 0);
