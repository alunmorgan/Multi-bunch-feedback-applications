...
set_variable([system_name pv_names.tails.triggers.MEM.disarm], 1)

for trigger_ind = 1:length(trigger_inputs)
    trigger = trigger_inputs{trigger_ind};
    set_variable([pv_head pv_names.tails.triggers.MEM.(trigger).enable_status], 'Ignore');
    set_variable([pv_head pv_names.tails.triggers.MEM.(trigger).blanking_status], 'All');
end %for
...
set_variable([pv_head, pv_head pv_names.MEM.channel_select], 'ADC0/ADC1')
