...
for nd = length(data_requested):-1:1
    ...
    spec_data{nd}  = mbf_spectrum_analysis(data_requested{nd});
...
%Forcing results to use 3 dimensions to allow accumulation and averaging over 
% experiments.
fnames = fieldnames(spec_data{1});
for sbd = 1:length(fnames)
    spec_data_temp.(fnames{sbd}) = NaN(1,1,2);
