...
from Common import constructDatastampedFilename, treeGen
...
def saveToArchive(root_string, data, graph_handles):
    ...
    save_name = os.path.join(root_string, data['time'][0], mth, dy, ''.join((data['filename'], '.mat')))
    savemat(save_name, data)
    if nargin > 3:
        for heaq in range(len(graph_handles)):
            if ishandle(graph_handles(heaq)):
                graph_save_name = os.path.join(root_string, data['time'][0], mth, dy, data['filename'])
                saveas(graph_handles(heaq), ''.join((graph_save_name, '_figure_', heaq, '.png')))
                saveas(graph_handles(heaq), ''.join((graph_save_name, '_figure_', heaq, '.fig')))
