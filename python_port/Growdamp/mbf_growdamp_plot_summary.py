def mbfGrowdampPlotSummary(poly_data, frequency_shifts):
    ...
    passive_data = -squeeze(poly_data[1][0]))
    active_data = -squeeze(poly_data[2][0]))

    passive_errors = NaN(len(passive_data),1)
    passive_errors(isnan(passive_data)) = 0
    active_errors = NaN(len(active_data),1)
    active_errors(isnan(active_data)) = 0

    #  NEED TO CONVERT TO MATPLOTLIB TODO
    figure
    ax1 = subplot(3,1,1:2)
    plot(x_plt_axis, circshift(passive_data, -harmonic_number/2, 1), 'b')
    hold on
    plot(x_plt_axis, circshift(active_data, -harmonic_number/2, 1), 'g')
    plot(x_plt_axis, zeros(len(x_plt_axis),1), 'r:')
    ...
    ax2 = subplot(3,1,3)
    plot(x_plt_axis, circshift(frequency_shifts, -harmonic_number/2, 1))
    xlim([x_plt_axis(1) x_plt_axis(end)])
